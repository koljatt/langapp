...
import { CURRICULUM } from "../data/index.js";
import { boxOf, unitStats, KNOWN_BOX } from "../lib/srs.js";
import { escapeHtml } from "../lib/text.js";
import { say, canSpeak } from "../lib/speech.js";
...
export function renderUnit() {
  ...
  h += `<div class="eyebrow num">${st.known} / ${st.total} osattu${st.due ? ` · ${st.due} kerrattavaa` : ""}</div>`;
  h += '<button class="btn" data-action="practice">Harjoittele tätä jaksoa</button>';
  ...
  for (const [it, fi, note] of u.items) {
    const b = boxOf(s, `${it}|${fi}`);
    const cls = b < 0 ? "" : b < 2 ? "l1" : b < KNOWN_BOX ? "l2" : "l3";
    const level = b < 0 ? "ei vielä aloitettu" : `laatikko ${b}`;
    h += `<div class="row">
      ...
      <span style="display:flex;align-items:center;gap:8px">
        ...
        ${canSpeak ? `<button class="spk" data-say="${escapeHtml(it)}" aria-label="Kuuntele: ${escapeHtml(it)}">${SPEAKER()}</button>` : ""}
      </span>
    </div>`;
  }
  h += "</div></div>";
